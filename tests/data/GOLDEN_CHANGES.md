...
| `real-freeipa-capture/dirsrv-down`, `real-freeipa-capture/dirsrv-stopped`, `dns/named-down` | `related_to_titles` (and the console "related to:") of the crashed/unexplained-checks entry | Slice 1 hardening, red-team round 4: it now names only the diagnosis that actually explains it ("dirsrv is not running"; "named failed to start"), no longer every problem in that pack (v0.1.3 also listed the unrelated CS.cfg file-mode finding, and the "records broken" symptom). |
