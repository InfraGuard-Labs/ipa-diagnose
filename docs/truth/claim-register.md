...
| C06 | File mode fix removes permissions only and is verified | LIVE | P1 (real CS.cfg 0664 quirk) | container `/data` layout | "On a fresh container install, ipa-healthcheck's CS.cfg 0664-vs-0660 finding got a permission-removing `chmod o-r`, preceded by read-only CONFIRM FIRST commands; verify reported RESOLVED." | "fixes file permission problems" |
| C07 | Group mismatch on a non-key file: fix applied and verified | LIVE | F1 (`/etc/ipa/ca.crt` chgrp) | one file | "A deliberately wrong group on /etc/ipa/ca.crt got `chgrp -h root`, verified afterwards." | - |
| C08 | No fix for non-IPA accounts, hard-linked files, key material | LIVE (+ FIXTURE for the rest) | F2, F3, F4 | only these three live | "Changing the file's group to `nobody`, adding a second hard link, or changing the group of the RA agent key all produced no fix." | "can never suggest an unsafe file change" |
